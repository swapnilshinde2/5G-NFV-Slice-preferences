...
Cp=[Cp; Cp; Cp; Cp; Cp; Cp; Cp]*10^(-3)
LayerCP_MMPA=LayerCP_MMPA*10^(-3)
LayerCP_MPA=LayerCP_MPA*10^(-3)
LayerCP_GA=LayerCP_GA*10^(-3)
z=1:1:Z;
...
for ttt=1:3
    ...
    for z=1:Z
        ...
        subplot(S,Z,(Z*(ttt-1))+z)
           % PosVec = Ax.Position;
    %Ax.Position = PosVec+[0 -0.005 0 0.005];
        %subplot('Position',pos1)
        b={bar3([1 2],data([1 end],:), 0.9,'stacked')};
        view(-70.5,10)
        hold on
        for k=2:(size(data,1)-1)
            %pos1 = [0.1+((k-1)*ww) 0.3 ww 0.5];
            b{k}=bar3([k-1 k],data([end k],:),0.9,'stacked');
            %subplot('Position',pos1)
        end
