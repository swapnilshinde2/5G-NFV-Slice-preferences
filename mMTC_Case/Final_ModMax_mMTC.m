...
Z=7;
for z=1:Z
    [F,Fup, Fcp, S, La, P, u, U,Actf,U_t,L_s,r_s,L_c,maxdup,maxdcp,K,Cp,Ol,t0,dels,Dtup,Dtcp,deltup,eta1,eta, idpr,DLatency]  =Input_mMTC(z);
  ...
  if c<=0
        if ceq==0
            ...
            fprintf( ' \n Does Follow Constraints for %d M Users with Cost Function Value: %d',DLatency(1,z)*10^(-3), objsum)
        else
            fprintf('\n Does Not Follow Constraints for %d M Users with Cost Function Value: %d',DLatency(1,z)*10^(-3), objsum);
        end
    else
        fprintf('\n Does Not Follow Constraints for %d M Users with Cost Function Value: %d',DLatency(1,z)*10^(-3), objsum);
    end
...
maxdcp=maxdcp*10^(3);
LayerCP_MMPA=LayerCP_MMPA*10^(-15);
Cp=Cp*10^(-15);
